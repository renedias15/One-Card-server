...
def get_card(card_id):
    try:
        cur = mysql.connection.cursor()
        cur.execute("SELECT * FROM users LEFT OUTER JOIN selectedPlan ON users.secret = selectedPlan.card_id where users.secret = %s ORDER by selectedPlan.id desc LIMIT 1;", (card_id,))
        card = cur.fetchone()
        cur.close()
        if card:
